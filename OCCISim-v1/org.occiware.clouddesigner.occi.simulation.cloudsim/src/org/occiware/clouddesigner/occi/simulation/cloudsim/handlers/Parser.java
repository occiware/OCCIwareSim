...
import org.occiware.clouddesigner.occi.simulation.impl.HostImpl;
import org.occiware.clouddesigner.occi.simulation.impl.VmImpl;
...
public class Parser {
	...
	private VM_Config VMFromResource(Resource resource){
		...
		{			
			if(m.getMixin().getTerm().equals("vm"))
			{
				VmImpl vm = (VmImpl) m;
				id_vm = vm.getIdvm();
				mips = 1000*vm.getMips();
				numberOfPes = vm.getCores();
				ram = vm.getRam();
				bw = vm.getBw();
				size = vm.getSize();
				vmm = vm.getVmm();
				cloudletScheduler = "CloudletSchedulerTimeShared";//To modify 
				ram_max = vm.getRamMaxVm();
				ram_min = vm.getRamMinVm();
				elastic_vm = vm.getElasticVm();
				os = vm.getOs();
				irdto = vm.getIrdto();
				dto =vm.getDto();
				dti = vm.getDti();
	...
	private Host_Config HostFromResource(Resource resource){
		...
		for(MixinBase mixin : resource.getParts()) {
			...
			if(mixin.getMixin().getName().equals("host"))
			{
				...
				id_host = as.getIdHost();
				mips = 1000*as.getMips();
				core = as.getCores();
				ram = as.getRam();//MB
				bw = as.getBw();
				storage = as.getStorage();//MB
				ramProvisioner = (as.getRamProvisioner() == null) ? "RamProvisionerSimple" : as.getRamProvisioner().toString();
				bwProvisioner = (as.getBwProvisioner() == null) ? "BwProvisionerSimple" : as.getBwProvisioner().toString();
				vmScheduler = (as.getVmScheduler() == null) ? "VmSchedulerTimeShared": as.getVmScheduler().toString();
				peProvisioner = (as.getPeProvisioner() == null) ? "PeProvisionerSimple": as.getPeProvisioner().toString();
				elastic_host = as.getElasticHost();
				ram_max_host = as.getRamMaxHost();
				ram_min_host = as.getRamMinHost();
				mips_max_host = as.getMipsMaxHost();
	...
	public class VM_Config implements Entity{
		...
		public VM_Config(String id, List<String> idTarget, int id_vm, int numberOfPes, int ram, double mips,
				...
				String elastic, int ram_min, int ram_max, String os, double irdto, 
